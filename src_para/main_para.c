...
store_pixels( char * filename, animated_gif * image )
{
    ...
    #pragma omp parallel for private(j, k, moy) shared(image, colormap, n_colors, error_occurred)
        ...
        if ( f == GRAPHICS_EXT_FUNC_CODE )
        {
            ...
            if ( tr_color >= 0 && tr_color < 255 )
            {
                ...
                if ( found == -1 ) 
                {
                        ...
                        {
                            fprintf( stderr, 
                                    "Error: Found too many colors inside the image P1\n"
                                ) ;
                            error_occurred = 1;
                        }
                        else 
    ...
    #pragma omp parallel for private(i, j, k, moy) shared(image, colormap, n_colors, error_occurred)
                ...
                if ( tr_color >= 0 && tr_color < 255 )
                {
                    ...
                    if ( found == -1 ) 
                    {
                        #pragma omp critical
                        {
                            ...
                            {
                                fprintf( stderr, 
                                        "Error: Found too many colors inside the image P2 \n"
                                       ) ;
                                error_occurred = 1;
                            }
                            else 
    ...
    // Removed the duplicate block of code here
    ...
    // Create a flattened array of pixels to process in parallel
    struct {
        int img_idx;
        int pixel_idx;
        pixel *pix;
    ...
    work_items = (typeof(work_items))malloc(total_pixels * sizeof(*work_items));
    if (work_items == NULL) {
        fprintf(stderr, "Error: Unable to allocate memory for work items\n");
        free(colormap);
        return 0;
    }
    ...
    for (i = 0; i < image->n_images; i++) {
        for (j = 0; j < image->width[i] * image->height[i]; j++) {
            work_items[idx].img_idx = i;
            work_items[idx].pixel_idx = j;
            work_items[idx].pix = &p[i][j];
            idx++;
        }
    }

    #pragma omp parallel for private(i, k) shared(work_items, total_pixels, n_colors, colormap, error_occurred)
    for (i = 0; i < total_pixels; i++) {
        int found = 0;
        for (k = 0; k < n_colors; k++) {
            if (work_items[i].pix->r == colormap[k].Red &&
                work_items[i].pix->g == colormap[k].Green &&
                work_items[i].pix->b == colormap[k].Blue) {
                found = 1;
                break;
            }
        }
        ...
        if (found == 0) {
            ...
            {
                if (n_colors >= 256) {
                    fprintf(stderr, "Error: Found too many colors inside the image P3\n");
                    error_occurred = 1;
                }
                else {
#if SOBELF_DEBUG
                    printf("[DEBUG] Found new %d color (%d,%d,%d)\n",
                            n_colors, work_items[i].pix->r, work_items[i].pix->g, work_items[i].pix->b);
#endif
                    colormap[n_colors].Red = work_items[i].pix->r;
                    colormap[n_colors].Green = work_items[i].pix->g;
                    colormap[n_colors].Blue = work_items[i].pix->b;
    ...
    #pragma omp parallel for private(i, k) shared(work_items, total_pixels, image, error_occurred)
    for (i = 0; i < total_pixels; i++) {
        int found_index = -1;
        for (k = 0; k < n_colors; k++) {
            if (work_items[i].pix->r == image->g->SColorMap->Colors[k].Red &&
                work_items[i].pix->g == image->g->SColorMap->Colors[k].Green &&
                work_items[i].pix->b == image->g->SColorMap->Colors[k].Blue) {
                found_index = k;
                break;
            }
        }
        ...
        if (found_index == -1) {
            ...
            image->g->SavedImages[work_items[i].img_idx].RasterBits[work_items[i].pixel_idx] = found_index;
        }
    }
...
// /////// Sequential version of store_pixels
// int
// store_pixels( char * filename, animated_gif * image )
// {
//     int n_colors = 0 ;
